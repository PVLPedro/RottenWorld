...
@section('content')
    <main class="relative bg-bg-secondary min-h-[calc(100dvh-120px)] sm:h-[calc(100dvh-120px)] p-1 lg:p-4 gap-1 lg:gap-4 flex flex-col justify-center sm:grid sm:grid-cols-2 lg:flex lg:flex-row flex-wrap">
        <section class="bg-bg-primary w-full flex justify-center h-15 p-2 rounded-lg col-span-2">
        ...
        <section id="characters" class="row-span-2 h-50 sm:h-[calc(100dvh-192px)] lg:h-[calc(100dvh-228px)] lg:flex-2 bg-bg-primary flex flex-col overflow-hidden rounded-lg">
        ...
        <section class="overflow-y-auto h-75 sm:h-auto lg:h-[calc(100dvh-228px)] lg:flex-3 relative bg-bg-primary rounded-lg p-4 *:my-4 [&_h1]:text-2xl [&_h1,h2]:font-bold [&_hr]:border [&_hr]:border-bg-tertiary">
            @if($selected)
                @php
                ...
                @endphp
                <a href="{{ route('sheets.edit', $selected) }}" class="w-10 sticky float-right top-4 bg-bg-tertiary hover:bg-bg-tertiary-hover cursor-pointer p-2 rounded-lg">
        ...
        <section class="overflow-y-auto h-75 sm:h-auto lg:h-[calc(100dvh-228px)] lg:flex-3 relative bg-bg-primary rounded-lg p-4 *:my-4 [&_h1]:text-2xl [&_h1,h2]:font-bold [&_hr]:border [&_hr]:border-bg-tertiary">
            @if ($selected)
                <a href="{{ route('caracteristicas.edit', $selected) }}" class="w-10 sticky float-right top-4 bg-bg-tertiary hover:bg-bg-tertiary-hover cursor-pointer p-2 rounded-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/><path d="m15 5 4 4"/></svg>
                </a>
                @if ($selected->caracteristicas)
                    ...
                    @php
                        ...
                        $ASPV = ($s->AFOR + $s->ACON) * 4 + 10 + (2 * $s->QDDM) + (2 * $s->AVON);
